#!/bin/zsh
# Rend une selection de Valentin a partir du cadrage envoye depuis l'outil.
#   ./rendre-val.sh 2
...
python3 $S/rendre-depuis-points.py "$json[1]" --in $D --out $F \
  --srt "$M/Horizontal/_captions/$nom.srt" \
  --titre "$T" \
  --sortie "$M/Vertical/Selection Valentin (Opus Clip)/$nom.mp4"
